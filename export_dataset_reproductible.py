...
PATH_DATAPAPERS_CLEAN = os.path.expanduser(
    "~/Documents/Stage/sentences_embeddings/Exports/datapapers_clean.csv"
)
...
def split_data(
    data, size_historic, size_context, size_novelty, theme, fix_seed=9583
):
    ...
    idx_all = random.sample(
        idx_no_novelty, size_historic + size_context - size_novelty
    )
    idx_historic = idx_all[0:size_historic]
    idx_context = idx_all[size_historic:] + random.sample(
        idx_novelty, size_novelty
    )
...
def main():
    ...
    if without_preprocessing:
        ...
        try:
            ...
            data = data.drop(
                ["id", "conf", "title", "author", "year", "eq", "conf_short"],
                axis=1,
            )
    ...
    for seed in SEEDS:
        ...
        for exp in SAMPLES_LIST:
            ...
            size_novelty = exp[2]
            logger.info(
                f"historique : {size_historic}, contexte : {size_context}, nouveauté : {size_novelty}"
            )

            data_historic, data_context = split_data(
                data,
                size_historic=size_historic,
                size_context=size_context,
                size_novelty=size_novelty,
                theme=theme,
                fix_seed=seed,
            )
            if without_preprocessing:
                data_historic.to_csv(
                    f"Exports/datapapers_fixed/historic_{size_historic}_{size_context}_{size_novelty}_s{seed}_{theme}_datapapers.csv",
                    sep="\t",
                )
                data_context.to_csv(
                    f"Exports/datapapers_fixed/context_{size_historic}_{size_context}_{size_novelty}_s{seed}_{theme}_datapapers.csv",
                    sep="\t",
                )
            else:
                data_historic.to_csv(
                    f"Exports/datapapers_fixed/historic_{size_historic}_{size_context}_{size_novelty}_s{seed}_{theme}_datapapers_clean.csv",
                    sep="\t",
                )
                data_context.to_csv(
                    f"Exports/datapapers_fixed/context_{size_historic}_{size_context}_{size_novelty}_s{seed}_{theme}_datapapers_clean.csv",
                    sep="\t",
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script d'export des jeux de données découpés en historique/contexte"
    )
    parser.add_argument(
        "--debug",
