...
def mat_conf(OBS, PRED):
    ...
    fp = sum(
        [1 if obs == 0 and pred == 1 else 0 for obs, pred in zip(OBS, PRED)]
    )
    fn = sum(
        [1 if obs == 1 and pred == 0 else 0 for obs, pred in zip(OBS, PRED)]
    )
    vp = sum(
        [1 if obs == 1 and pred == 1 else 0 for obs, pred in zip(OBS, PRED)]
    )
    vn = sum(
        [1 if obs == 0 and pred == 0 else 0 for obs, pred in zip(OBS, PRED)]
    )
