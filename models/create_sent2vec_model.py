...
def main():
    ...
    if file not in ["datapapers", "nytdata"]:
        ...
        df = pd.read_csv(
            os.path.expanduser("~/Documents/Données/datapapers.csv"),
            sep="\t",
            encoding="utf-8",
        )
        ...
        for index, row in tqdm(
            df.iterrows(), dynamic_ncols=True, total=taille_df
        ):
            sentences = [
                x.strip() for x in re.split("\.", row["abstract"].lower())
            ]
        ...
        df = pd.read_csv(
            os.path.expanduser(
                "~/Documents/Données/data_big_category_long.csv"
            ),
            sep="\t",
            encoding="utf-8",
        )
        ...
        for index, row in tqdm(
            ...
            sentences = [
                x.strip() for x in re.split("\.", row["texts"].lower())
            ]
            list_sentences = list_sentences + sentences
